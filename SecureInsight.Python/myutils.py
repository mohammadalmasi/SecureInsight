...
def getblocksVisual(mode,sourcecode, badpositions,commentareas, fulllength,step, nr,w2v_model,model,
                    threshold,name,save_blocks_visual_path, number_of_example):
      ...
      color = "white"
      # # Find lines containing the comment '// Your C# code here'
      # comment_lines = [line.strip() for line in lines if '//' in line]
      # color = "grey"
...
#            print(list(focusarea))
            if focusarea[0] == "\n":
              ...
              bbox = d.textbbox((xpos, ypos), focusarea[1:])  # Get bounding box
              xpos = xpos + (bbox[2] - bbox[0])  # Calculate width
            else:
              d.text((xpos, ypos), focusarea, fill=color)
              #xpos = xpos + d.textsize(focusarea)[0]
              bbox = d.textbbox((xpos, ypos), focusarea)  # Get bounding box
              xpos = xpos + (bbox[2] - bbox[0])  # Calculate width

        except Exception as e:
          print(f"{Style.BRIGHT}{Fore.YELLOW}Exception WOW: {Style.RESET_ALL}{Style.BRIGHT}{e}")
          ...
          # if error_count == 14:
          #   print(f"{Style.BRIGHT}{Fore.GREEN}Saved png. {Style.RESET_ALL}")
            #break
      ...
      for i in range(0,number_of_example):
        if not os.path.isfile(f'{save_blocks_visual_path}//demo_' + mode + "_" + str(i) +"_"+ name + '.png'):
                img.save(f'{save_blocks_visual_path}//demo_' + mode + "_" + str(i) + "_" + name + '.png')    
                print("saved png.")
                break
      return blocks
